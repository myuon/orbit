...
fn print_benchmark_report(results: &[BenchmarkResult], timestamp: &str, commit_hash: &str) {
    ...
    // Performance by program section
    println!("\n## Performance by Program\n");
    
    // Group results by program
    let mut programs: std::collections::HashMap<String, Vec<&BenchmarkResult>> = std::collections::HashMap::new();
    for result in results {
        programs.entry(result.program.clone()).or_insert(Vec::new()).push(result);
    }
    
    for (program_name, program_results) in programs {
        println!("### {}\n", program_name);
        println!("| Test Size | Orbit Time (ms) | Rust Time (ms) | Speedup | Status |");
        println!("|-----------|-----------------|----------------|---------|--------|");
...
fn run_orbit_with_timeout(file_path: &str, timeout_secs: u64) -> Result<(i64, Duration), String> {
    ...
    // Timeout reached
    let _ = child.kill();
    Err("Timeout: execution exceeded 10 seconds".to_string())
}
...
fn main() {
    ...
    let benchmark_configs = vec![
        ...
        BenchmarkConfig {
            ...
            test_sizes: [1000, 5000, 10000, 20000, 30000],
        },
        BenchmarkConfig {
    ...
    for config in benchmark_configs {
        ...
        for &test_size in &config.test_sizes {
            // Measure Rust reference implementation  
            let rust_start = Instant::now();
            let rust_result = (config.rust_func)(test_size);
            ...
            let orbit_result = match temp_file_result {
                Ok(temp_file_path) => {
                    let result = run_orbit_with_timeout(&temp_file_path, 10);
                    // Clean up temp file
            ...
            let benchmark_result = match orbit_result {
