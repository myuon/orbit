...
mod tests {
    ...
    fn test_file_execution() {
        ...
        let test_cases = vec![
            ("tests/testcase/simple_function.ob", "5"),
            ("tests/testcase/arithmetic_expressions.ob", "14"),
            ("tests/testcase/variable_usage.ob", "30"),
        ];
        ...
        for (file_path, expected) in test_cases {
            if Path::new(file_path).exists() {
                ...
                match result {
                    Some(value) => assert_eq!(value.to_string(), expected, "Failed for file: {}", file_path),
