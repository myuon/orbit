...
impl TypeChecker {
    ...
    fn register_struct(&mut self, struct_decl: &PositionedStructDecl) -> Result<()> {
        ...
        if !struct_decl.value.type_params.is_empty() {
            ...
            self.structs
                .insert(struct_decl.value.name.clone(), struct_type);
        ...
        self.struct_fields
            .insert(struct_decl.value.name.clone(), fields);
        ...
        match &mut expr.value {
                ...
                match &container_value_type {
                    ...
                    Type::Pointer(element_type) => {
                        if !index_type.is_compatible_with(&Type::Int) {
                            bail_with_position!(
                                index.span.clone(),
                                "Pointer index must be number, got {}",
                                index_type
                            );
                        }
                        Ok(*element_type.clone())
            ...
            Expr::StructNew {
                ...
                *kind = updated_kind;
                ...
                let struct_fields =
                    self.resolve_struct_fields_from_type_with_span(type_name, &expr.span)?;
                ...
                match object_type {
                    Type::String => {
                        ...
                        if field == "length" {
                            ...
                            bail_with_position!(
                                object.span.clone(),
                                "Field '{}' not found in string type",
                                field
                            )
                        }
                    }
                    Type::Struct {
                        ...
                        if struct_name == "array" && args.len() == 1 && args[0] == Type::Byte {
                            if field == "length" {
                                ...
                                bail_with_position!(
                                    object.span.clone(),
                                    "Field '{}' not found in array(byte) type",
                                    field
                                );
                            }
                        }
            ...
            Expr::Alloc { element_type, size } => {
                ...
                if !size_type.is_compatible_with(&Type::Int) {
                    bail_with_position!(
                        size.span.clone(),
                        "Allocation size must be a number, got {}",
                        size_type
                    );
                }
            ...
            Expr::MethodCall {
                ...
                if let Some(obj) = object {
                    ...
                    match object_type {
                        Type::String => {
                            ...
                            if method == "length" {
                                ...
                                bail_with_position!(
                                    obj.span.clone(),
                                    "Method '{}' not found in string type",
                                    method
                                );
                            }
                        }
                        Type::Struct {
                            ...
                            if method == "length" {
                                ...
                                bail_with_position!(
                                    obj.span.clone(),
                                    "Method '{}' not found in array(byte) type",
                                    method
                                );
                            }
                        }
                    ...
                    bail_with_position!(
                        expr.span.clone(),
                        "MethodCall must have either object or object_type specified"
                    )
                }
            }
        }
    ...
    fn resolve_struct_fields_from_type_with_span(
        &self,
        type_info: &Type,
        span: &Span,
